...
> Scope: current simplified architecture, deployed with company Nginx + backend + MySQL.

| ID | Priority | Risk | Impact | Mitigation | Status | Acceptance |
|---|---|---|---|---|---|---|
| P0-01 | P0 | Frontend build failure | Cannot publish frontend static files | Keep Element Plus locale injection consistent | done | `npm run build` passes |
...
| P2-02 | P2 | Nginx config drift | Company Nginx routes frontend but not API | Keep `deploy/nginx/qrcode-channel.conf` as the reference config | doing | `/api/health` is reachable through Nginx |
